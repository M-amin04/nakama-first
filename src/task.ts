...
export function validateAndParse<T>(ctx: nkruntime.Context, payload: string): ValidatedRequest<T> {
  ...
  try {
    const input = JSON.parse(payload) as T;
    return { userId, input };
  } catch (error) {
...
export const afterAuthenticateDevice: nkruntime.AfterHookFunction<
  nkruntime.Session,
...
> = function (
  ...
  try {
    nk.accountUpdateId(
      userId,
      randomNumericUsername,
      randomNumricDisplayname,
      null,
      null,
      null,
      null,
...
export const getShopItemsRpc: nkruntime.RpcFunction = function (
  ...
  try {
    ...
    if (record.length === 0) return JSON.stringify({ items: [] });
...
export const setActiveItemRpc: nkruntime.RpcFunction = function (
  ...
  const { userId, input } = validateAndParse<{ itemType: string; itemId: string }>(ctx, payload);
...
export const buyItemRpc: nkruntime.RpcFunction = function (
  ...
  try {
    ...
    logger.info(
      `buy is success! Storage Acks: ${result.storageWriteAcks.length}, Wallet Acks: ${result.walletUpdateAcks.length}`,
    );
